...
- **上传解析**：支持 PDF 订单、Excel 客户资料（含图号）；同名文件不可重复上传，重复订单号自动跳过；异步后台解析，实时进度轮询
- **图号自动关联**：PDF 明细按「客户图号 / 客户新图号」精确匹配，未命中时按「外径×壁厚」规格前缀唯一匹配
- **订单管理**：列表筛选（关键词 / 客户 / 关联状态 / 推送状态）、订单详情、手动推送
